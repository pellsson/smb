...
OperModeExecutionTree:
      ...
      .word GameModeValueSubs
      .word VictoryModeValueMain
      .word GameOverSubs
...
VictoryModeValueMain:
          jsr VictoryModeValueSubroutines ;run victory mode subroutines in order
...
VictoryModeValueSubroutines:
    lda WorldNumber               ;run different list of subroutines if on world 8
    cmp #World8
    beq VictoryModeValueSubsForW8      ;note that world D will also run second set of subs
VictoryModeValueSubroutines2:
    lda OperMode_Task             ;after running the first two subs in the first set
    jsr JumpEngine

    .word BridgeCollapse
    .word SetupVictoryModeValue
...
VictoryModeValueSubsForW8:
    ...
    .word VictoryModeValueDiskRoutines
...
SetupVictoryModeValue:
      ...
      bne @VictoryModeValue                  ; if so - skip to 2j code
      ldy WorldNumber                   ; get current world
      cpy #World8                       ; are we in world 8?
      beq @VictoryModeValue                  ; yes - skip to 2j code
...
.ifdef ANN
         lda #CHR_ANN_VICTORY    ;load victory mode CHR data
.else
         ldy WorldNumber
         lda WorldBits,y
         ora CompletedWorlds     ;set bit according to the world the player was in
         sta CompletedWorlds
         lda #CHR_LL_VICTORY    ;load victory mode CHR data
.endif
         sta MMC5_CHRBank+2
...
.ifdef ANN
           ...
           BankJSR BANK_ANNDATA, ANN_LoadAreaPointer
.else
           lda WorldNumber
           clc
...
NoPast9:   sta WorldNumber           ;update the world number
           BankJSR BANK_LLDATA, LL_LoadAreaPointer
.endif
...
.ifdef ANN
            BankJSR BANK_ANNDATA, ANN_GetAreaDataAddrs
.else
            BankJSR BANK_LLDATA, LL_GetAreaDataAddrs
.endif
...
GameModeValueSubs:
      ...
      .word GameModeValueDiskRoutines
      .word InitializeArea
.ifdef ANN
...
.ifdef ANN
NotW9:    BankJSR BANK_ANNDATA, ANN_LoadAreaPointer       ;get new level pointer
.else
NotW9:    BankJSR BANK_LLDATA, LL_LoadAreaPointer       ;get new level pointer
.endif
...
EvenDgs: ldy #$00               ;if they are both even, set state and counter
         ...
         bne SetFWC
NoFWks:  ldy #$00
SetFWC:  lda #$ff               ;otherwise set value for no fireworks
...
SetWDest: tay
          ...
          ldx #0
.ifdef ANN
          BankJSR BANK_ANNDATA, ANN_GetAreaPointer
.else
...
PrintWorld9Msgs:
       ...
       cmp #GameOverModeValue         ;note this routine only runs after world 8 and replaces
